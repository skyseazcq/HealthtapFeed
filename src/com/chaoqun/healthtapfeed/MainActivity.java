...
public class MainActivity extends ListActivity {
	...
	protected void onListItemClick(ListView l, View v, int position, long id) {
		...
		Intent intent = new Intent(this, FeedAnswer.class);
		intent.putExtra("oneFeed", mFeeds.get(position));
	...
	public void handleFeedResponse() {
		...
		if (mJsonArray == null) {
			...
			try {
				...
				for (int i = 0; i < mJsonArray.length(); i++) {
					...
					String answer = user_answer.getString("long_answer");
					String snapshot = user_answer.getString("snapshot");
					String avatar_normalPath = user_answer.getJSONObject(
							"avatar").getString("normal");

					OneFeed oneFeed = new OneFeed(last_name, question,
							imagePath);
					oneFeed.setFullName(name);
					oneFeed.setAnswer(answer);
					oneFeed.setSnapshot(snapshot);
					oneFeed.setAvatar(avatar_normalPath);

					mFeeds.add(0, oneFeed);
		...
		++RefreshCnt;
		/*
		 * while(++RefreshCnt < 2) { FeedGetTask getFeedTask = new
		 * FeedGetTask(); getFeedTask.execute(); }
		 */
	}
	...
	private class LoginPostTask extends AsyncTask<Object, Void, Boolean> {
		...
		protected Boolean doInBackground(Object... args) {
			...
			HttpPost httppost = new HttpPost(
					"http://www.healthtap.com/people/sign_in");
			...
			try {
				...
				List<NameValuePair> nameValuePairs = new ArrayList<NameValuePair>(
						3);
				nameValuePairs.add(new BasicNameValuePair("person[email]",
						email));
				nameValuePairs.add(new BasicNameValuePair("person[password]",
						password));
				nameValuePairs
						.add(new BasicNameValuePair("login_from_hash", ""));
	...
	private class FeedGetTask extends AsyncTask<Object, Void, JSONArray> {
		...
		protected JSONArray doInBackground(Object... args) {
			...
			HttpGet request = new HttpGet(
					"https://www.healthtap.com/polymorphic_activities.json?v2=1&page="
							+ pageNum + "&per_page=8");
